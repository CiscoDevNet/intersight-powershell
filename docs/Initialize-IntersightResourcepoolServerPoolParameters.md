...
```yaml
Type: ResourcepoolServerPoolParameters.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
### -ManagementMode
The platform for which the servers in resource pool are applicable. It can either be a server that is operating in standalone mode or which is attached to a Fabric Interconnect managed by Intersight.\n* `IntersightStandalone` - Intersight Standalone mode of operation.\n* `UCSM` - Unified Computing System Manager mode of operation.\n* `Intersight` - Intersight managed mode of operation.

```yaml
Type: ResourcepoolServerPoolParameters.ManagementModeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightResourcepoolServerPoolParameters
```
