...
Initialize-IntersightFabricPortIdentifier [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-AggregatePortId< long>][-ClassId< FabricPortIdentifier.ClassIdEnum>][-ObjectType< FabricPortIdentifier.ObjectTypeEnum>][-PortId< long>][-SlotId< long>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: FabricPortIdentifier.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: FabricPortIdentifier.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightFabricPortIdentifier
```
