...
# Initialize-IntersightBootPrecisionPolicy
...
```yaml
Type: System.Collections.Generic.List`1[Model.BootDeviceBase]
Parameter Sets: (All)
Aliases:
...
```yaml
Type: BootPrecisionPolicy.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
### -ConfiguredBootMode
Sets the BIOS boot mode. UEFI uses the GUID Partition Table (GPT) whereas Legacy mode uses the MBR partitioning scheme. Unified Edge servers support only UEFI boot mode. Legacy mode is not supported on these platforms. To apply this setting, Please reboot the server.\n* `Uefi` - UEFI mode uses the GUID Partition Table (GPT) to locate EFI Service Partitions to boot from.\n* `Legacy` - Legacy mode refers to the traditional process of booting from BIOS. Legacy mode uses the MBR to locate the bootloader. Not supported on Unified Edge Server.
...
```yaml
Type: BootPrecisionPolicy.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightBootPrecisionPolicy
```
