...
Initialize-IntersightFcpoolBlock [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< FcpoolBlock.ClassIdEnum>][-From< string>][-ObjectType< FcpoolBlock.ObjectTypeEnum>][-Size< long>][-To< string>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: FcpoolBlock.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightFcpoolBlock
```
