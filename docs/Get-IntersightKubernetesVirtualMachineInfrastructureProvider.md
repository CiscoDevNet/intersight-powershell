...
## SYNOPSIS
Get the KubernetesVirtualMachineInfrastructureProvider

## SYNTAX

### CmdletParam (Default)
```
Get-IntersightKubernetesVirtualMachineInfrastructureProvider [-Name <String>] [-Organization <OrganizationOrganizationRelationship>]
 [<CommonParameters>]
```

### QueryParam
```
Get-IntersightKubernetesVirtualMachineInfrastructureProvider [-Count <Boolean>] [-InlineCount <String>] [-Select <String>] [-Filter <String>]
...
### Example 1
```powershell
PS C:\> $result = Get-IntersightKubernetesVirtualMachineInfrastructureProvider
```
Above example returns the list of KubernetesVirtualMachineInfrastructureProvider.

### Example 2
```powershell
PS C:\> $result = Get-IntersightKubernetesVirtualMachineInfrastructureProvider -Name "MoName"
```
Above example returns the KubernetesVirtualMachineInfrastructureProvider matched with name.

### Example 3
```powershell
PS C:\> $result = Get-IntersightKubernetesVirtualMachineInfrastructureProvider -Organization $organization
```
Above example returns the list of KubernetesVirtualMachineInfrastructureProvider available under the $organization .

### Example 4
```powershell
PS C:\> $result = Get-IntersightKubernetesVirtualMachineInfrastructureProvider -InlineCount allpages
```
Above example returns the list of KubernetesVirtualMachineInfrastructureProvider along with count.
...
### -Name
Specifies the name of the KubernetesVirtualMachineInfrastructureProvider.
...
### -Organization
Specifies the organization, to fetch the KubernetesVirtualMachineInfrastructureProvider under it.
