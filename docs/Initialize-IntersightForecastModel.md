...
Initialize-IntersightForecastModel [-Accuracy< float>][-AdditionalProperties< Dictionary<string,object>>][-ClassId< ForecastModel.ClassIdEnum>][-ModelData< List<decimal>>][-ModelType< ForecastModel.ModelTypeEnum>][-ObjectType< ForecastModel.ObjectTypeEnum>]
...
```yaml
Type: ForecastModel.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightForecastModel
```
