...
Initialize-IntersightWorkflowFailureEndTask [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< WorkflowFailureEndTask.ClassIdEnum>][-Description< string>][-Label< string>][-Name< string>][-ObjectType< WorkflowFailureEndTask.ObjectTypeEnum>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: WorkflowFailureEndTask.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: WorkflowFailureEndTask.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightWorkflowFailureEndTask
```
