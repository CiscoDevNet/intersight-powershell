...
# Initialize-IntersightFirmwareSecureRouterUpgrade
...
Initialize-IntersightFirmwareSecureRouterUpgrade [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< FirmwareSecureRouterUpgrade.ClassIdEnum>][-DirectDownload< Model.FirmwareDirectDownload>][-Distributable< Model.FirmwareDistributableRelationship>][-FileServer< Model.SoftwarerepositoryFileServer>][-Moid< string>][-NetworkSecureRouter< Model.NetworkSecureRouterRelationship>][-NetworkShare< Model.FirmwareNetworkShare>][-ObjectType< FirmwareSecureRouterUpgrade.ObjectTypeEnum>][-Release< Model.SoftwarerepositoryReleaseRelationship>][-SkipEstimateImpact< bool>][-Status< FirmwareSecureRouterUpgrade.StatusEnum>][-Tags< System.Collections.Generic.List`1[Model.MoTag]>][-UpgradeType< FirmwareSecureRouterUpgrade.UpgradeTypeEnum>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: FirmwareSecureRouterUpgrade.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
### -NetworkSecureRouter
A reference to a networkSecureRouter resource.\nWhen the $expand query parameter is specified, the referenced resource is returned inline.

 Note:- To get the relationship object pass the MO to the cmdlet Get-IntersightMoMoRef 
or use the cmdlet Initialize-IntersightMoMoRef.

```yaml
Type: Model.NetworkSecureRouterRelationship
Parameter Sets: (All)
Aliases:
...
```yaml
Type: FirmwareSecureRouterUpgrade.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: FirmwareSecureRouterUpgrade.StatusEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: FirmwareSecureRouterUpgrade.UpgradeTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightFirmwareSecureRouterUpgrade
```
