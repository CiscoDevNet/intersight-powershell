...
Initialize-IntersightWorkloadDefinitionMapper [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< WorkloadDefinitionMapper.ClassIdEnum>][-DefinitionName< string>][-ObjectType< WorkloadDefinitionMapper.ObjectTypeEnum>][-Organization< Model.MoMoRef>][-UsePreferredVersion< bool>][-Version< long>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: WorkloadDefinitionMapper.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: WorkloadDefinitionMapper.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightWorkloadDefinitionMapper
```
