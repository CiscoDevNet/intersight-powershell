...
# Initialize-IntersightAssetToken
...
Initialize-IntersightAssetToken [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< AssetToken.ClassIdEnum>][-ObjectType< AssetToken.ObjectTypeEnum>][-Signature< byte[]>][-TimeStamp< DateTime>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: AssetToken.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightAssetToken
```
