...
# Initialize-IntersightFabricPortMode
...
Initialize-IntersightFabricPortMode [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< FabricPortMode.ClassIdEnum>][-CustomMode< FabricPortMode.CustomModeEnum>][-Moid< string>][-ObjectType< FabricPortMode.ObjectTypeEnum>][-PortIdEnd< long>][-PortIdStart< long>][-PortPolicy< Model.FabricPortPolicyRelationship>][-SlotId< long>][-Tags< System.Collections.Generic.List`1[Model.MoTag]>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: FabricPortMode.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: FabricPortMode.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightFabricPortMode
```
