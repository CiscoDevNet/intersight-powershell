...
```powershell
$KubernetesVirtualMachineNodeProfileRelationship = Initialize-IntersightKubernetesVirtualMachineNodeProfileRelationship  -ClassId null `
 -ObjectType null `
 -Moid null `
 -Selector null `
 -Link null `
...
```powershell
$KubernetesVirtualMachineNodeProfileRelationship | ConvertTo-JSON
```
