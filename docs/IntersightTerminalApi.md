...
# Read a 'terminal.AuditLog' resource.
try {
     $Result = Get-IntersightTerminalAuditLogByMoid -Moid $Moid
} catch {
...
# Read a 'terminal.AuditLog' resource.
try {
     $Result = Get-IntersightTerminalAuditLogList -VarFilter $VarFilter -Orderby $Orderby -Top $Top -Skip $Skip -Select $Select -Expand $Expand -Apply $Apply -Count $Count -Inlinecount $Inlinecount -At $At -Tags $Tags
} catch {
