...
# Initialize-IntersightConnectorEventLog
...
Initialize-IntersightConnectorEventLog [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< ConnectorEventLog.ClassIdEnum>][-EncryptedAesKey< string>][-EncryptionKey< string>][-ObjectType< ConnectorEventLog.ObjectTypeEnum>][-SecureProperties< object>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: ConnectorEventLog.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: ConnectorEventLog.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightConnectorEventLog
```
