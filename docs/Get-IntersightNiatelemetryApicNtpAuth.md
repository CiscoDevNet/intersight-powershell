...
## SYNOPSIS
Get the NiatelemetryApicNtpAuth

## SYNTAX

### CmdletParam (Default)
```
Get-IntersightNiatelemetryApicNtpAuth [-Name <String>] [-Organization <OrganizationOrganizationRelationship>]
 [<CommonParameters>]
```

### QueryParam
```
Get-IntersightNiatelemetryApicNtpAuth [-Count <Boolean>] [-InlineCount <String>] [-Select <String>] [-Filter <String>]
 [-Expand <String>] [-Skip <Int32>] [-Top <Int32>] [-At <String>] [-Orderby <String>] [-Apply <String>]
 [-Tag <String>] [<CommonParameters>]
```
## DESCRIPTION
Read a &apos;NiatelemetryApicNtpAuth&apos; resource.
...
### Example 1
```powershell
PS C:\> $result = Get-IntersightNiatelemetryApicNtpAuth
```
Above example returns the list of NiatelemetryApicNtpAuth.

### Example 2
```powershell
PS C:\> $result = Get-IntersightNiatelemetryApicNtpAuth -Name "MoName"
```
Above example returns the NiatelemetryApicNtpAuth matched with name.

### Example 3
```powershell
PS C:\> $result = Get-IntersightNiatelemetryApicNtpAuth -Organization $organization
```
Above example returns the list of NiatelemetryApicNtpAuth available under the $organization .

### Example 4
```powershell
PS C:\> $result = Get-IntersightNiatelemetryApicNtpAuth -InlineCount allpages
```
Above example returns the list of NiatelemetryApicNtpAuth along with count.
...
### -Name
Specifies the name of the NiatelemetryApicNtpAuth.
...
### -Organization
Specifies the organization, to fetch the NiatelemetryApicNtpAuth under it.
