...
Initialize-IntersightAssetHttpConnection [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-CertificateAuthority< string>][-ClassId< AssetHttpConnection.ClassIdEnum>][-Credential< Model.AssetCredential>][-IsSecure< bool>][-ManagementAddress< string>][-ObjectType< AssetHttpConnection.ObjectTypeEnum>][-Port< long>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: AssetHttpConnection.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: AssetHttpConnection.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightAssetHttpConnection
```
