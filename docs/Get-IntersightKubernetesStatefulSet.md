...
## SYNOPSIS
Get the KubernetesStatefulSet

## SYNTAX

### CmdletParam (Default)
```
Get-IntersightKubernetesStatefulSet [-Name <String>] [-Organization <OrganizationOrganizationRelationship>]
 [<CommonParameters>]
```

### QueryParam
```
Get-IntersightKubernetesStatefulSet [-Count <Boolean>] [-InlineCount <String>] [-Select <String>] [-Filter <String>]
 [-Expand <String>] [-Skip <Int32>] [-Top <Int32>] [-At <String>] [-Orderby <String>] [-Apply <String>]
 [-Tag <String>] [<CommonParameters>]
```
## DESCRIPTION
Read a &amp;apos;KubernetesStatefulSet&amp;apos; resource.,
...
### Example 1
```powershell
PS C:\> $result = Get-IntersightKubernetesStatefulSet
```
Above example returns the list of KubernetesStatefulSet.

### Example 2
```powershell
PS C:\> $result = Get-IntersightKubernetesStatefulSet -Name "MoName"
```
Above example returns the KubernetesStatefulSet matched with name.

### Example 3
```powershell
PS C:\> $result = Get-IntersightKubernetesStatefulSet -Organization $organization
```
Above example returns the list of KubernetesStatefulSet available under the $organization .

### Example 4
```powershell
PS C:\> $result = Get-IntersightKubernetesStatefulSet -InlineCount allpages
```
Above example returns the list of KubernetesStatefulSet along with count.
...
### -Name
Specifies the name of the KubernetesStatefulSet.
...
### -Organization
Specifies the organization, to fetch the KubernetesStatefulSet under it.
...
[Remove-IntersightKubernetesStatefulSet](./Remove-IntersightKubernetesStatefulSet.md)
