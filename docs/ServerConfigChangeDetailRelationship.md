...
```powershell
$ServerConfigChangeDetailRelationship = Initialize-IntersightServerConfigChangeDetailRelationship  -ClassId null `
 -ObjectType null `
 -Moid null `
 -Selector null `
 -Link null `
...
```powershell
$ServerConfigChangeDetailRelationship | ConvertTo-JSON
```
