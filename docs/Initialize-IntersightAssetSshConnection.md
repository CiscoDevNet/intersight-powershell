...
Initialize-IntersightAssetSshConnection [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][-ClassId< AssetSshConnection.ClassIdEnum>][-Credential< Model.AssetCredential>][-ManagementAddress< string>][-ObjectType< AssetSshConnection.ObjectTypeEnum>][-Port< long>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: AssetSshConnection.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: AssetSshConnection.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightAssetSshConnection
```
