...
```powershell
$EquipmentRackEnclosureRelationship = Initialize-IntersightEquipmentRackEnclosureRelationship  -ClassId null `
 -ObjectType null `
 -Moid null `
 -Selector null `
 -Link null `
...
```powershell
$EquipmentRackEnclosureRelationship | ConvertTo-JSON
```
