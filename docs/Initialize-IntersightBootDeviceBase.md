...
Initialize-IntersightBootDeviceBase [-AdditionalProperties< System.Collections.Generic.Dictionary`2[string,object]>][[-ClassId]< BootDeviceBase.ClassIdEnum>][-Enabled< bool>][-Name< string>][[-ObjectType]< BootDeviceBase.ObjectTypeEnum>][-Json< SwitchParameter>][-WithHttpInfo< SwitchParameter>]
...
```yaml
Type: BootDeviceBase.ClassIdEnum
Parameter Sets: (All)
Aliases:
...
```yaml
Type: BootDeviceBase.ObjectTypeEnum
Parameter Sets: (All)
Aliases:
...
### Example 1
```powershell
PS C:\> Initialize-IntersightBootDeviceBase
```
