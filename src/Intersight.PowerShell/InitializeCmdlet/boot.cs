...
namespace Intersight.PowerShell
{
    ...
    /// <summary>
    /// <para type="synopsis">This is the cmdlet to Initialize BootFlexMmc.</para>
    /// </summary>
    [Cmdlet(VerbsData.Initialize, "IntersightBootFlexMmc")]
    public class InitializeIntersightBootFlexMmc : PSCmdlet
    {
        public InitializeIntersightBootFlexMmc()
        {
            ClassId = BootFlexMmc.ClassIdEnum.BootFlexMmc;
            ObjectType = BootFlexMmc.ObjectTypeEnum.BootFlexMmc;
            Subtype = BootFlexMmc.SubtypeEnum.None;
        ...
        public BootFlexMmc.ClassIdEnum ClassId
        {
            get;
            set;
        }
        ...
        {
            PSUtils.ProcessRelationshipParam(this.MyInvocation.BoundParameters);
            Intersight.Model.BootFlexMmc initObject = new Intersight.Model.BootFlexMmc();
            if (this.MyInvocation.BoundParameters.ContainsKey("AdditionalProperties"))
            {
                initObject.AdditionalProperties = this.AdditionalProperties;
            }
    ...
    public class InitializeIntersightBootHttp : PSCmdlet
    {
        ...
        // <summary>
        /// <para type="description">"Enter the port ID of the adapter for the virtual ethernet interface. Valid values are 0 to 255."</para>
        /// </summary>
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, ValueFromPipeline = false)]
        [ValidateRange(-1, 255)]
        public long Port
    ...
    public class InitializeIntersightBootLocalCdd : PSCmdlet
    {
        ...
        {
            ClassId = BootLocalCdd.ClassIdEnum.BootLocalCdd;
            ObjectType = BootLocalCdd.ObjectTypeEnum.BootLocalCdd;
    ...
    public class InitializeIntersightBootPrecisionPolicy : PSCmdlet
    {
        ...
        // <summary>
        /// <para type="description">"Sets the BIOS boot mode. UEFI uses the GUID Partition Table (GPT) whereas Legacy mode uses the MBR partitioning scheme. Unified Edge servers support only UEFI boot mode. Legacy mode is not supported on these platforms. To apply this setting, Please reboot the server.\n* `Uefi` - UEFI mode uses the GUID Partition Table (GPT) to locate EFI Service Partitions to boot from.\n* `Legacy` - Legacy mode refers to the traditional process of booting from BIOS. Legacy mode uses the MBR to locate the bootloader. Not supported on Unified Edge Server."</para>
        /// </summary>
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, ValueFromPipeline = false)]
    ...
    public class InitializeIntersightBootSan : PSCmdlet
    {
        ...
        // <summary>
        /// <para type="description">"The Logical Unit Number (LUN) of the device. For SAN boot configuration to be deployed on a server with 1300 family of Cisco VIC adapters, the recommendation is for the boot LUN to be numbered as 0 to ensure that LUN is mounted as the first disk from which the server boots."</para>
        /// </summary>
        [Parameter(Mandatory = false, ValueFromPipelineByPropertyName = true, ValueFromPipeline = false)]
    ...
    public class InitializeIntersightBootUefiShell : PSCmdlet
    {
        ...
        {
            ClassId = BootUefiShell.ClassIdEnum.BootUefiShell;
            ObjectType = BootUefiShell.ObjectTypeEnum.BootUefiShell;
