...
function Initialize-IntersightHyperflexSysConfigPolicy {
    ...
    Process {
        ...
        if (!$DnsServers -and $DnsServers.length -gt 3) {
            throw "invalid value for 'DnsServers', number of items must be less than or equal to 3."
        }

        if (!$DnsServers -and $DnsServers.length -lt 1) {
            throw "invalid value for 'DnsServers', number of items must be greater than or equal to 1."
        }

        if (!$NtpServers -and $NtpServers.length -gt 3) {
            throw "invalid value for 'NtpServers', number of items must be less than or equal to 3."
        }

        if (!$NtpServers -and $NtpServers.length -lt 1) {
            throw "invalid value for 'NtpServers', number of items must be greater than or equal to 1."
        }
...
function ConvertFrom-IntersightJsonToHyperflexSysConfigPolicy {
    ...
    Process {
        ...
        $AllProperties = ("ClassId", "ObjectType", "AccountMoid", "CreateTime", "DomainGroupMoid", "ModTime", "Moid", "Owners", "SharedScope", "Tags", "VersionContext", "_0_ClusterReplicationNetworkPolicy", "Ancestors", "Parent", "PermissionResources", "DisplayNames", "Description", "Name", "DnsDomainName", "DnsServers", "NtpServers", "Timezone", "ClusterProfiles", "Organization")
