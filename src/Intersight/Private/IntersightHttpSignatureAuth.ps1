...
.OUTPUTS
    Hashtable
#>
function Get-IntersightHttpSignedHeader {
    param(
        [string]$Method,
    ...
    $httpSigningConfiguration = Get-IntersightConfigurationHttpSigning
    $Digest = $null
    
    #get the body digest
    $bodyHash = Get-IntersightStringHash -String $Body -HashName $httpSigningConfiguration.HashAlgorithm
    ...
    foreach ($headerItem in $httpSigningConfiguration.HttpSigningHeader) {
        ...
        elseif ($headerItem -eq $HEADER_CREATED) {
            $created = Get-IntersightUnixTime -Date $dateTime -TotalTime TotalSeconds
        ...
        elseif ($headerItem -eq $HEADER_EXPIRES) {
            ...
            $expireEpocTime = Get-IntersightUnixTime -Date $expire -TotalTime TotalSeconds
    ...
    $KeyType = Get-IntersightKeyTypeFromFile -KeyFilePath $httpSigningConfiguration.KeyFilePath

    if ($keyType -eq "RSA") {
        $headerSignatureStr = Get-IntersightRSASignature -PrivateKeyFilePath $httpSigningConfiguration.KeyFilePath `
    ...
    elseif ($KeyType -eq "EC") {
        $headerSignatureStr = Get-IntersightECDSASignature -ECKeyFilePath $httpSigningConfiguration.KeyFilePath `
            -DataToSign $signatureHashString `
            -HashAlgorithmName $httpSigningConfiguration.HashAlgorithm `
            -KeyPassPhrase $httpSigningConfiguration.KeyPassPhrase
    }
    #Depricated
    <#$cryptographicScheme = Get-IntersightCryptographicScheme -SigningAlgorithm $httpSigningConfiguration.SigningAlgorithm `
...
.OUTPUTS
    Base64String
#>
function Get-IntersightRSASignature {
    Param(
    ...
    try {
        ...
        if ($PSVersionTable.PSVersion.Major -ge 7) {
            ...
            $rsa_provider_path = Join-Path -Path $PSScriptRoot -ChildPath "IntersightRSAEncryptionProvider.cs"
...
.OUTPUTS
    Base64String
#>
function Get-IntersightECDSASignature {
    param(
    ...
    # Not supported on Linux/Mac: $ecdsa = [System.Security.Cryptography.ECDsaCng]::New()
    ...
    if ($HashAlgorithmName -eq "sha512") {
        ...
        $signedBytes = $ecdsa.SignData($DataToSign, [System.Security.Cryptography.HashAlgorithmName]::New('SHA512'))
    }
    else {
        $ecdsa.HashAlgorithm = [System.Security.Cryptography.HashAlgorithm]::Sha256
        $signedBytes = $ecdsa.SignData($DataToSign, [System.Security.Cryptography.HashAlgorithmName]::New('SHA256'))
    }
...
String 
#>
Function Get-IntersightStringHash {
    param(
        [Parameter(Mandatory = $true)]
        [AllowEmptyString()]
        [string]$String,
...
Integer
#>
function Get-IntersightUnixTime {
    param(
        [Parameter(Mandatory = $true)]
        [DateTime]$Date,
        [Parameter(Mandatory = $false)]
...
function Get-IntersightCryptographicScheme {
    param(
        [Parameter(Mandatory = $true)]
        [string]$SigningAlgorithm,
        [Parameter(Mandatory = $true)]
...
String
#>
function Get-IntersightKeyTypeFromFile {
    param(
        [Parameter(Mandatory = $true)]
        [string]$KeyFilePath
    )
